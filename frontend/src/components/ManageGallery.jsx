...
const ManageGallery = () => {
  ...
  const userClubId = typeof window !== 'undefined' ? localStorage.getItem('club_id') : null;
  ...
  useEffect(() => {
    // Set default club_id when clubs are loaded
  ...
  if (!isMounted || !role) {
    return (
      <div className="min-h-screen bg-[#f6efe6]">
        <Navbar />
        <div className="max-w-[900px] mx-auto px-6 py-24 text-center">Loading...</div>
        <Footer />
      </div>
    );
  }
  ...
  const handleAddImage = async (e) => {
    ...
    try {
      ...
      setForm({ url: '', text: '', club_id: role !== 'SUPER_ADMIN' && userClubId ? Number(userClubId) : (clubs.length > 0 ? clubs[0].id : '') });
  ...
  // Filter gallery based on user role
  const displayGallery = role === 'SUPER_ADMIN' ? gallery : gallery.filter(img => img.club_id === Number(userClubId));
  ...
  return (
    <div className="min-h-screen bg-[#f6efe6]">
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-24">
        <h1 className="text-4xl font-bold text-[#12202b] mb-8 text-center">Manage Gallery</h1>

        {error && <div className="bg-red-100 text-red-700 p-4 rounded mb-6">{error}</div>}
        {successMessage && <div className="bg-green-100 text-green-700 p-4 rounded mb-6">{successMessage}</div>}

        {/* Add Image Form */}
        <div className="bg-white rounded-lg shadow-lg p-8 mb-12">
          <h2 className="text-2xl font-bold text-[#12202b] mb-6">Add Gallery Image</h2>
          
          <form onSubmit={handleAddImage} className="space-y-4">
            <div>
              <label className="block text-sm font-semibold text-[#12202b] mb-2">Image URL *</label>
              <input
                type="url"
                name="url"
                value={form.url}
            ...
            <div>
              <label className="block text-sm font-semibold text-[#12202b] mb-2">Image Description (Optional)</label>
              <textarea
                name="text"
                value={form.text}
                onChange={handleChange}
        ...
        <div className="bg-white rounded-lg shadow-lg p-8">
          ...
          {displayGallery.length === 0 ? (
            <p className="text-center text-[#7b6f61] py-8">No gallery images yet. Add one to get started!</p>
          ) : (
              ...
              {displayGallery.map(image => (
                <div key={image.id} className="border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition">
                  ...
                  <div className="p-4">
                    ...
                    {image.text && <p className="text-sm text-[#7b6f61] mb-3 line-clamp-2">{image.text}</p>}
                    <p className="text-xs text-gray-500 mb-3 truncate">{image.url}</p>
                    <button
